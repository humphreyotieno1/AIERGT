  ...
  static async sendUserVerificationEmail(user: any): Promise<boolean> {
    const template: EmailTemplate = {
      ...
      html: `
        ...
        <p><a href="${process.env.NEXTAUTH_URL}/dashboard" style="background: #2D5016; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Access Your Dashboard</a></p>
        <p>Best regards,<br>The AIERGT Team</p>
      `
    }
  ...
  static async sendPasswordResetEmail(user: any, resetUrl: string): Promise<boolean> {
    const template: EmailTemplate = {
      ...
      html: `
        ...
        <p><a href="${resetUrl}" style="background: #2D5016; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block;">Reset Password</a></p>
